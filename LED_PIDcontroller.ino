// it's controls a LED base in the LDR feedback

#include <PID_v1.h>

const int photores = 0; // Photo resistor input
const int pot = 1; // Potentiometer input
...
void setup(){
  ...
  Input = map(lightLevel, 0, 1024, 0, 255); //Change read scale to analogout scale
...
void loop(){
  lightLevel = analogRead(photores); //Read in light level
  Setpoint = map(analogRead(pot), 0, 1024, 0, 255); //Read our setpointlightLevel = analogRead(photores); //Get the light level
  ...
  if(now - lastMessage > serialPing) { //If it has been long enough give ussome info on serial
